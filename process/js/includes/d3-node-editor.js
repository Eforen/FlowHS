...
var template = function (locals) {
  var pug_html = "",
    ...
    pug_html = pug_html + "<div class=\"socket output {{output.socket.id}}\" al-pick-output title=\"{{output.socket.name}}\n{{output.socket.hint}}\"></div></div>";
    ...
    pug_html = pug_html + "<div class=\"socket input {{input.socket.id}} {{input.multipleConnections?'multiple':''}}\" al-pick-input title=\"{{input.socket.name}}\n{{input.socket.hint}}\"></div>";
...
var template$1 = function (locals) {
  var pug_html = "",
      pug_mixins = {},
      pug_interp;var pug_debug_filename, pug_debug_line;try {
    var pug_debug_sources = {};
...
var Utils = function () {
    ...
    createClass(Utils, null, [{
        ...
        value: function isValidData(data) {
            return typeof data.id === 'string' && this.isValidId(data.id) && data.nodes instanceof Object && (!data.groups || data.groups instanceof Object);
        }
    }, {
        key: 'isValidId',
...
function Group$1(scope, el, expression, env) {
    var _this = this;
...
function PickInput(scope, el, expression, env) {
    ...
    d3.select(el).on('mousedown', function () {
        ...
        if (!input.multipleConnections && input.hasConnection()) _this.editor.removeConnection(input.connections[0]);else if (_this.pickedOutput.connectedTo(input)) {
            var connections = input.connections.filter(function (c) {
                return c.output === _this.pickedOutput;
            });

            _this.editor.removeConnection(connections[0]);
        }
...
function declareDirectives(view) {

    alight.directives.al.node = Node$1.bind(view);

    alight.directives.al.group = Group$1.bind(view);
...
var template$2 = function (locals) {
  var pug_html = "",
      pug_mixins = {},
      pug_interp;var pug_debug_filename, pug_debug_line;try {
    var pug_debug_sources = {};
    ...
    pug_html = pug_html + "<path class=\"connection\" al-repeat=\"path in editor.paths\" :d=\"path.d\" :class.selected=\"path.selected\"></path></svg>";
    pug_html = pug_html + "<!-- Nodes-->";
    pug_html = pug_html + "<div class=\"node {{editor.selected.contains(node)?'selected':''}} {{node.title.toLowerCase().replace('/ /g','-')}}\" al-repeat=\"node in editor.nodes\" :style.transform=\"&quot;translate(&quot;+node.position[0]+&quot;px,&quot;+node.position[1]+&quot;px)&quot;\" al-node>";
...
var EditorView = function () {
    function EditorView(editor, container, menu) {
        ...
        this.contextMenu.default.onClick = function (item) {

            if (item instanceof Component) {
                var node = item.newNode();

                item.builder(node);
                _this.editor.addNode(node, true);
            } else item();
        ...
        this.zoom = d3.zoom().on('zoom', function () {
            var t = _this.transform = d3.event.transform;
            var style = 'translate(' + t.x + 'px, ' + t.y + 'px) scale(' + t.k + ')';

            _this.view.style('transform', style);
        });
    ...
    createClass(EditorView, [{
        ...
        value: function areaClick() {
            if (this.pickedOutput !== null && !d3.event.ctrlKey) this.pickedOutput = null;else if (this.contextMenu.visible) this.contextMenu.hide();else this.contextMenu.show(d3.event.clientX, d3.event.clientY);
            this.update();
        }
    }, {
        key: 'zoomAt',
        value: function zoomAt(nodes) {
            ...
            (_zoom = this.zoom).translateTo.apply(_zoom, [this.container].concat(toConsumableArray(bbox.getCenter())));
...
var Selected = function () {
    ...
    createClass(Selected, [{
        ...
        value: function add(item) {
            ...
            if (!(item instanceof Node || item instanceof Group)) {
...
var NodeEditor = function () {
    ...
    createClass(NodeEditor, [{
        ...
        value: function keyDown() {
            switch (d3.event.keyCode) {
                ...
                case 71:
                    ...
                    if (nodes.length > 0) this.addGroup(new Group('Group', { nodes: nodes }));
        ...
        value: function fromJSON(json) {
            ...
            if (!checking.success) throw new Error(checking.msg);
            ...
            Object.keys(json.nodes).forEach(function (id) {
                ...
                var component = _this2.components.find(function (c) {
                    return c.name === node.title;
                });
