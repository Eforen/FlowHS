...
export const tests_CMDRenameFlow = () => {
    describe('CMD: CMDRenameFlow', () => {
        let store: Store<RootState>
        let flow: Flow
        
        let makeState = (overloads: any) => {
         return _.merge({ 
            "version":"1.0.0",
            "commands":{ 
               "historyCount":20,
        ...
        let prep = () => {
            ...
            store.replaceState(makeState(makeState({
               "flows":{
                  "flows":{
                     "4bc08543-15b3-4b5a-bfaf-fda8396181da":{
                        "title":"startName"
        ...
        it('Do/Undo', () => {
           let randomName = uuid.v4()
            let cmd = new CMDRenameFlow("4bc08543-15b3-4b5a-bfaf-fda8396181da", randomName) //Was e1a2381b-35d0-4155-8659-a53e88e0600e
            //console.log(JSON.stringify(store.state))
            cmd.exe(store.dispatch, store.state)
            //console.log(JSON.stringify(store.state))
            expect(store.state).deep.equal(makeState({
               "flows":{
                  "flows":{
                     "4bc08543-15b3-4b5a-bfaf-fda8396181da":{
                        "title":randomName
